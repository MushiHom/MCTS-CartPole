...
GAME_ACTIONS = [0, 1]
MCTS_POLICY_EXPLORE = 3 # MCTS exploring constant: the higher, the more reliable, but slower in execution time
c = 1
...
class Node:
    ...
    def create_child(self):
        ...
        for action, game in zip(actions, games):
            ...
            child[action] = Node(game, terminated, self, observation, action)                        
...
if __name__ == '__main__':
    ...
    for e in range(episodes):
        ...
        while not (terminated or truncated):
            ...
            if (terminated or truncated):
                if (e + 1) % 10 == 0:
                    print(f"Episode: {e + 1}, Reward: {reward_e}")
                game.close()
                break
                
        if e > 30 and np.mean(rewards[-10:]) == 500:
            print("Early exit")
            break
